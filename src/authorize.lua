...
-- check service
if not service then
  if not service_uri and not service_host then
    ngx.log(ngx.DEBUG, "==== no valid service uri or service host provided")
  else
    ngx.log(ngx.DEBUG, "==== service not set for service uri: " .. service_uri)
    ngx.log(ngx.DEBUG, "==== service not set for service host: " .. service_host)
  end
...
ngx.log(ngx.DEBUG, "Service name passed to account service: " .. service)
...
ngx.log(ngx.DEBUG, "REDIRECTING TO ORIGINAL URL: " .. original_url)
